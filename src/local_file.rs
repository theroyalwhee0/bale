...
impl LocalFileHeader {
    ...
    #[must_use]
    pub fn new(size: u32, crc32: u32, mtime: DosDateTime) -> Self {
        Self {
...
mod tests {
    ...
    /// Stride includes header (30) plus fixed filename field (256).
    #[test]
    fn stride_is_header_plus_path() {
        assert_eq!(LocalFileHeader::STRIDE, 30 + 256);
    }

    /// New headers must have the correct ZIP signature.
    #[test]
    fn new_header_has_correct_signature() {
        let header = LocalFileHeader::new(100, 0x12345678, DosDateTime::default());
    ...
    fn roundtrip() {
        ...
        let header = LocalFileHeader::new(1024, 0xDEADBEEF, mtime);
